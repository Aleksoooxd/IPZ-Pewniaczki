...
from src.flask_app.app.models import (
    FootballMatch, MatchForm, MatchStats, Predicted, League, Season, Team
)


def create_match_dataframe_sql(session: Session, engine: Engine) -> pd.DataFrame:
    ...
    stmt_main = (
        select(
            ...
            FootballMatch.is_surprise_h.label('is_surprise_h'),
            FootballMatch.is_surprise_d.label('is_surprise_d'),
            FootballMatch.is_surprise_a.label('is_surprise_a'),
...
def drop_non_predictive_columns(df: pd.DataFrame) -> pd.DataFrame:
    non_predictive_cols = [
        ...
        'home_matchday', 'away_matchday', 'league', 'season',
        'home_goals', 'away_goals', 'home_elo_change', 'away_elo_change', 'is_surprise',
    ]
...
def predict_future_matches_nn(model, scaler, features_list, result_map, session: Session, engine: Engine):
    matches_to_predict_df = create_match_dataframe_sql(session, engine)
    ...
    if not existing_predictions_df.empty:
        ...
        matches_to_predict_df = matches_to_predict_df[
            ~matches_to_predict_df['match_id'].isin(predicted_match_ids)
        ]
    ...
    if 'consensus' in matches_to_predict_df_processed.columns and \
            matches_to_predict_df_processed['consensus'].dtype == 'object':
        matches_to_predict_df_processed = pd.get_dummies(
            matches_to_predict_df_processed, columns=['consensus'], prefix='consensus'
        )
    ...
    current_consensus_dummy_cols = [
        col for col in matches_to_predict_df_processed.columns if col.startswith('consensus_')
    ]
    extra_dummy_cols_to_drop = [
        col for col in current_consensus_dummy_cols if col not in expected_consensus_dummy_cols
    ]
    ...
    with torch.no_grad():
        ...
        for i, inputs in enumerate(tqdm(predict_loader, desc="Processing matches", unit="batch")):
            ...
            batch_original_indices = original_indices[
                i * predict_loader.batch_size: (i + 1) * predict_loader.batch_size
            ]
            ...
            for j in range(len(batch_match_ids)):
                predictions_to_save.append({
                    ...
                    'predicted_result': label_map[predicted_classes[j].item()],
                    'confidence': confidences[j].item()
                })

    with session.begin_nested():
        for pred_data in tqdm(predictions_to_save, desc="Saving predictions", unit="prediction"):
            existing_prediction = session.execute(
                select(Predicted).filter_by(match_id=pred_data['match_id'])
            ).scalar_one_or_none()

            if existing_prediction:
                existing_prediction.predicted_result = pred_data['predicted_result']
                existing_prediction.confidence = pred_data['confidence']
            else:
...
def predict(session: Session, engine: Engine):
    """
    Punkt wejścia do trenowania i predykcji.
    Przyjmuje sesję i engine z zewnątrz — bez zależności od Flask.
    """
    df = create_match_dataframe_sql(session, engine)
    ...
    features_list = [
        ...
        'home_h2h_matches', 'home_h2h_goals_for', 'home_h2h_goals_against',
        'home_h2h_last_5_points'
    ]
    ...
    df_predictions = predict_future_matches_nn(model, scaler, features_list, result_map, session, engine)
